...
#define AttackSound	""
#define HitSound	""
#define DeathSound	"sounds/DK_PlayerDeath.ogg"
#define WalkSound	"sounds/DK_Footstep_Left_03.ogg"
	...
	void WeaponCollisionEnter(CollisionEvent e)
	{
		...
		if (e.m_CollidingWith->m_GameObject->tag == Object::ObjectTags::Player)
		{
			if (e.m_ThisObject->m_GameObject->GetComponent<HitManager>()->RegisterHit(e.m_CollidingWith->m_GameObject))
			{
				...
				float Damage = e.m_ThisObject->m_GameObject->GetComponent<StatScript>()->GetAtkDamage();
				auto Stats = player->GetComponent<StatScript>();
				if (player->m_MyHash && player->m_Creator == *player->m_MyHash)
					printf("Client hit\n");
				if (player->GetComponent<NetPlayerComponent>()->GetState() == PlayerStates::Dead)
					return;
				if (Stats)
				{
					Stats->TakeDamage(Damage, e.m_ThisObject->m_GameObject);
					SoundSourceComponent* soundSourceComponent = player->GetComponent<SoundSourceComponent>();

					if (soundSourceComponent) {
						soundSourceComponent->SetSound(DataManager::GetInstance()->LoadSound("sound/DK_PlayerDamage.ogg"));
...
void NetEnemyComp::PacketUpdate(const float & deltaTime)
{
...
void NetEnemyComp::ExecuteState(const float & deltaTime)
{
	...
	switch (m_State)
	{
		case NetEnemyComp::EnemyStates::Idle:
		{
			//Check Player 
		...
		case NetEnemyComp::EnemyStates::Chase:
		{
				...
				if (temp != nullptr)
				{
					...
					if (glm::length2(m_GameObject->m_Transform[3] - target->m_Transform[3]) < (atkRange*atkRange))
					{
						if (timeSinceAttack > 1.0f)
						{
							...
							glm::vec3 temp = cross(x, glm::vec3(0, 1, 0));
							//nDir = glm::vec4(temp.x, temp.y, temp.z,0);
							...
							m_GameObject->m_Transform[2].x = temp.x;
							m_GameObject->m_Transform[2].y = temp.y;
							m_GameObject->m_Transform[2].z = temp.z;
		...
		case NetEnemyComp::EnemyStates::Search:
		{
		...
		case NetEnemyComp::EnemyStates::Attack:
		{
			//Attack player
			Attack(deltaTime);
		}break;

		case NetEnemyComp::EnemyStates::Stunned:
		{
		...
		case NetEnemyComp::EnemyStates::Dead:
		{
...
void NetEnemyComp::Stun(float deltaTime)
{
...
bool NetEnemyComp::CheckPlayerVision(const float & deltaTime)
{
...
void NetEnemyComp::EnemyPath(const float & deltaTime)
{
...
void NetEnemyComp::DealWithColorShit(float deltaTime)
{
...
void NetEnemyComp::Attack(const float & deltaTime)
{
...
void NetEnemyComp::Die()
{
	...
	RigidBodyComponent* rb = m_GameObject->GetComponent<RigidBodyComponent>();

	if (sc)
		sc->SetEnabled(false);
	if (rb)
		rb->SetVelocity(glm::vec3(0.0f, 0.0f, 0.0f));

	//PickupManager::GetInstance()->SpawnItemPickup(m_GameObject->m_Transform[3], (uint16_t)ItemIDs::ChickenDinner);
...
void NetEnemyComp::OnUpdate(float deltaTime)
{
...
void NetEnemyComp::OnStart()
{
	...
	HitManager* m_hitManager = new HitManager();
	m_GameObject->AddComponent(m_hitManager);

	m_RigidBody = m_GameObject->GetComponent<RigidBodyComponent>();
	m_StatScript = m_GameObject->GetComponent<StatScript>();
	m_StatScript->SetStunThreshold(100.0f);
	...
	m_soundSourceComp = new SoundSourceComponent();
	m_GameObject->AddComponent(m_soundSourceComp);
	m_SoundSource = m_GameObject->GetComponent<SoundSourceComponent>();
}

void NetEnemyComp::ServerUpdate(const float & deltaTime)
{
	//m_StatScript->TakeDamage(1, NULL);
}

void NetEnemyComp::ClientUpdate(const float & deltaTime)
{

}

void NetEnemyComp::HandleMessage(RakNet::BitStream * bs)
{
...
NetEnemyComp::EnemyStates NetEnemyComp::GetState()
{
	return m_State;
}

void NetEnemyComp::SetState(EnemyStates state)
{
	...
	if (state != m_State)
	{
		...
		switch (state)
		{
			case EnemyStates::Attack:
			{
				...
				if (m_SoundSource)
				{
					m_soundSourceComp->SetSound(DataManager::GetInstance()->LoadSound(AttackSound));
					m_soundSourceComp->Play();
				}
			...
			case EnemyStates::Chase:
			{
				...
				if (m_SoundSource)
				{
					m_soundSourceComp->SetSound(DataManager::GetInstance()->LoadSound(WalkSound));
			...
			case EnemyStates::Stunned:
			{
				...
				if (m_SoundSource) 
				{
					m_soundSourceComp->SetSound(DataManager::GetInstance()->LoadSound("sounds/DK_EnemyDamage.ogg"));
					m_soundSourceComp->Play();
				}
...
void NetEnemyComp::ExitState(EnemyStates newState)
{
	switch (m_State)
	{
		case EnemyStates::Idle:
		...
		{
			AtkCapsule->SetEnabled(false);
			m_GameObject->GetComponent<HitManager>()->ResetObjects();
			break;
		}
